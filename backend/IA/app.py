...
def summarize_long_text(text):
    ...
    try:
        ...
        if len(text.strip()) < 50:
            return "Document trop court pour générer un résumé significatif."

        # Si le texte est assez court, le résumer directement
        if len(text.split()) <= 400:
            try:
                summary = summarizer(text, max_length=100, min_length=20, do_sample=False)
                return summary[0]['summary_text']
            except:
                return f"Résumé partiel: {text[:300]}..."
        ...
        for i, chunk in enumerate(chunks[:5]):
            try:
                ...
                summaries.append(f"Section {i+1}: {chunk[:100]}...")

        if not summaries:
            return f"Résumé basique: {text[:300]}..."
...
def extract_text_from_pdf(pdf_file):
    ...
    try:
        with fitz.open(pdf_file) as doc:
            ...
            # Nettoyer le texte pour éviter les erreurs du modèle
            import re
